...
if(isset($_POST['change'])){
	if(strlen($_POST['n_password']) > 5){
		...
		$xml = new SimpleXMLElement('../../superusers/' . $_SESSION['username'] . '.xml', 0, true);
		if($old == $xml->password){
			if($new == $c_new){
				$xml->password = $new;
				$xml->asXML('../../superusers/' . $_SESSION['username'] . '.xml');
				header('Location: login.php');
				die;
			}
		}
...
<body>
	...
	<!-- Subnav -->
				...
				<div class="in minitext">
					...
					<p>This page will change your password for the BoxCtrl-VZ admin area.</p>
				</div>
